...
Suspend support not enabled.", true);
        ...
        private void HotkeysTimer_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            if (Global.SwipeProfiles)
            {
                // Build the ordered list of profile indices to cycle through.
                // If SwipeProfileList is empty, cycle all profiles (original behaviour).
                // Otherwise restrict to the user-chosen subset, preserving their display order.
                System.Collections.Generic.List<string> swipeFilter = Global.SwipeProfileList;
                ...
                foreach (CompositeDeviceModel item in conLvViewModel.ControllerCol)
                {
                    ...
                    if (slide == "left" || slide == "right")
                    {
                        Dispatcher.BeginInvoke((Action)(() =>
                        {
                            // Build filtered index list each tick so it stays current
                            System.Collections.Generic.List<int> candidateIndices;
                            if (swipeFilter == null || swipeFilter.Count == 0)
                            {
                                // All profiles
                                candidateIndices = System.Linq.Enumerable.Range(0, item.ProfileListCol.Count).ToList();
                            }
                            else
                            {
                                candidateIndices = new System.Collections.Generic.List<int>();
                                for (int pi = 0; pi < item.ProfileListCol.Count; pi++)
                                {
                                    if (swipeFilter.Contains(item.ProfileListCol[pi].Name))
                                        candidateIndices.Add(pi);
                                }
                                // Fallback: if the filter names don't match anything, use all
                                if (candidateIndices.Count == 0)
                                    candidateIndices = System.Linq.Enumerable.Range(0, item.ProfileListCol.Count).ToList();
                            }

                            // Find position of current profile in the candidate list
                            int posInCandidates = candidateIndices.IndexOf(item.SelectedIndex);
                            if (posInCandidates < 0)
                                posInCandidates = 0;

                            if (slide == "left")
                            {
                                int newPos = posInCandidates <= 0
                                    ? candidateIndices.Count - 1
                                    : posInCandidates - 1;
                                item.SelectedIndex = candidateIndices[newPos];
                            }
                            else // right
                            {
                                int newPos = posInCandidates >= candidateIndices.Count - 1
                                    ? 0
                                    : posInCandidates + 1;
                                item.SelectedIndex = candidateIndices[newPos];
                            }
        ...
        private void SwipeProfilesConfigBtn_Click(object sender, RoutedEventArgs e)
        {
            // Gather all profile names from the profile list
            var allProfiles = profileListHolder.ProfileListCol.Select(p => p.Name);
            var currentSelection = Global.SwipeProfileList ?? new System.Collections.Generic.List<string>();

            var dialog = new SwipeProfileSelectWindow(allProfiles, currentSelection);
            dialog.Owner = this;
            if (dialog.ShowDialog() == true)
            {
                ...
                settingsWrapVM.RaiseSwipeProfileListSummaryChanged();
            }
        }
