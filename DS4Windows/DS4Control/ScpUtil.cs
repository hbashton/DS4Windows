...
namespace DS4Windows
{
    ...
    public class Global
    {
        ...
        public static List<string> SwipeProfileList
        {
            ...
            set { m_Config.swipeProfileList = value; }
        }
    ...
    public class BackingStore
    {
        ...
        // List of profile names to cycle through on touchpad swipe. Empty = cycle all profiles.
        ...
        public bool LoadOld()
        {
            ...
            try
            {
                if (File.Exists(m_Profile))
                {
                    ...
                    try
                    {
                        Item = m_Xdoc.SelectSingleNode("/Profile/SwipeProfileList");
                        swipeProfileList = new List<string>();
                        if (Item != null && !string.IsNullOrEmpty(Item.InnerText))
                        {
                            foreach (string prof in Item.InnerText.Split(','))
                            {
                                string trimmed = prof.Trim();
                                if (!string.IsNullOrEmpty(trimmed))
                                    swipeProfileList.Add(trimmed);
                            }
                        }
                    }
                    catch { missingSetting = true; }
        ...
        public bool SaveOld()
        {
            ...
            XmlNode xmlSwipeProfileList = m_Xdoc.CreateNode(XmlNodeType.Element, "SwipeProfileList", null); xmlSwipeProfileList.InnerText = string.Join(",", swipeProfileList); rootElement.AppendChild(xmlSwipeProfileList);
