...
namespace DS4Windows.Tests
{
    ...
    public class DualSenseBluetoothAudioPacerTemplateTests
    {
        ...
        [TestMethod]
        public void MicrophoneReserveTransferIsFiniteAndReturnsToRationalCadence()
        {
            const long qpcFrequency = 10_000_000;
            const long startQpc = 1_000_000;
            long transferInterval = qpcFrequency *
            ...
            for (int index = 0; index <
                DualSenseBluetoothAudioPacer.
                    MicrophoneReserveTransferIntervals; index++)
            {
                scheduler.AdvanceAfterSend(previous);
                Assert.AreEqual(transferInterval,
                    scheduler.NextDeadlineQpc - previous);
                previous = scheduler.NextDeadlineQpc;
            }

            scheduler.AdvanceAfterSend(previous);
            long firstNominal = scheduler.NextDeadlineQpc - previous;
            Assert.IsTrue(firstNominal == 106_666 ||
                firstNominal == 106_667,
                "The finite transfer leaked into steady presentation cadence.");

            long nominalStart = scheduler.NextDeadlineQpc;
            for (int index = 0; index < 3000; index++)
            {
                scheduler.AdvanceAfterSend(scheduler.NextDeadlineQpc);
            }
            Assert.AreEqual(qpcFrequency * 32L,
                scheduler.NextDeadlineQpc - nominalStart,
                "The reserve transfer changed the rational long-run media clock.");
        }

        [TestMethod]
        public void MicrophoneReserveTransferMovesOnlyExistingLatencyRunway()
        {
            const double nominalIntervalMs = 32.0 / 3.0;
            double transferredMilliseconds =
                DualSenseBluetoothAudioPacer.
                    MicrophoneReserveTransferIntervals *
                (nominalIntervalMs -
                    DualSenseBluetoothAudioPacer.
                        MicrophoneReserveTransferIntervalMicroseconds /
                    1000.0);
            double hostFramesSpent = transferredMilliseconds /
                nominalIntervalMs;

            Assert.IsTrue(transferredMilliseconds >= 38.0 &&
                transferredMilliseconds < 39.0);
            Assert.IsTrue(hostFramesSpent < 4.0);
            Assert.IsTrue(DualSenseBluetoothAudioPacer.
                MicrophoneReserveTransferMinimumHostReports -
                    hostFramesSpent > 5.0,
                "The bounded transfer consumed the helper's host runway.");
        }

        [TestMethod]
        public void CancellingMicrophoneReserveTransferRestoresNextNativeGap()
        {
            const long qpcFrequency = 10_000_000;
            var scheduler = new DualSenseBluetoothAudioPacerScheduler(
                qpcFrequency);
            scheduler.Start(1_000_000);
            scheduler.BeginControllerReserveTransfer(23, 9_000);
