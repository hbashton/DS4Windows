...
namespace DS4Windows.InputDevices
{
    ...
    {
        private static readonly Logger micDiagLogger = LogManager.GetLogger("DualSenseMicDiag");
        ...
        private const int BluetoothInputBufferCount = 64;
        ...
        private const byte DualSensePowerSaveControlActiveBaseline = 0x0F;
        ...
        private const byte DualSenseAudioControl2SpeakerCompPreGain = 0x01;
        ...
        private int bluetoothMicrophoneInputSuppressed;
        private int bluetoothConsecutiveMicrophoneFrames;
        ...
        public long BluetoothMicrophoneSuppressedFrameCount =>
        ...
        private unsafe void ReadInput()
        {
            unchecked
            {
                ...
                NativeMethods.HidD_SetNumInputBuffers(hDevice.SafeReadHandle.DangerousGetHandle(),
                    conType == ConnectionType.BT ? BluetoothInputBufferCount : 3);
                if (conType == ConnectionType.BT)
                {
                    ...
                    if (conType == ConnectionType.BT)
                    {
                        ...
                        if (res == HidDevice.ReadStatus.Success)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    this.inputReportErrorCount = 0;
                                    RecordBluetoothNormalInputFrame();
                                }
                            }
                        }
                        else
        ...
        private void RecordBluetoothNormalInputFrame()
        {
            Interlocked.Exchange(ref bluetoothConsecutiveMicrophoneFrames, 0);
            Interlocked.Exchange(ref bluetoothLastNormalInputTimestamp, Stopwatch.GetTimestamp());
        }
        ...
        private void ApplyBluetoothMicrophoneStreamingRequest(byte[] combined)
        {
            ...
            if (Volatile.Read(ref bluetoothMicrophoneStreamingRequested) != 0 &&
                Volatile.Read(ref bluetoothMicrophoneInputSuppressed) == 0)
            {
                if (combined.Length > BluetoothCombinedAudioControlFlagsOffset)
                {
                    // DS5Dongle/DS5_Bridge verified this top-level 0x36
                    // audio-control byte as 0xFE without mic and 0xFF with
                    // mic. Preserve the known baseline flags, then set bit 0.
                    combined[BluetoothCombinedAudioControlFlagsOffset] =
                ...
                combined[BluetoothCombinedStateAudioControlOffset] =
                    (byte)((combined[BluetoothCombinedStateAudioControlOffset] &
                        ...
                        DualSenseAudioControlNoiseCancelEnable);
                if (combined.Length > BluetoothCombinedStateAudioControl2Offset)
                {
                    combined[BluetoothCombinedStateAudioControl2Offset] |=
                        DualSenseAudioControl2SpeakerCompPreGain;
                }
        ...
        /// <summary>
        /// Sends the controller's Bluetooth microphone stream-control packet.
        /// The proven HidBth path uses combined Bluetooth audio report 0x36,
        /// byte 4 bit 0. Mic-tagged input report 0x31 frames are diverted
        /// before normal controller parsing.
        /// </summary>
        public bool SetBluetoothMicrophoneStreaming(bool enabled, bool waitForWrite = false)
        {
            if (enabled && !BluetoothMicrophoneInputTransportAvailable)
            {
                ...
                LastBluetoothMicrophoneWriteStatus =
                    "Rejected: physical Bluetooth mic capture is not available in this build.";
                return false;
            }
        ...
        public void ResetBluetoothMicrophoneProbeStatistics()
        {
            ...
            Interlocked.Exchange(ref bluetoothMicrophoneSuppressedFrameCount, 0);
            Interlocked.Exchange(ref bluetoothConsecutiveMicrophoneFrames, 0);
            Interlocked.Exchange(ref bluetoothMicrophoneInputSuppressed, 0);
            Interlocked.Exchange(ref bluetoothLastNormalInputTimestamp, Stopwatch.GetTimestamp());
        ...
        private byte[] BuildBluetoothMicrophoneCombinedReport(bool enabled)
        {
            ...
            {
                if (latestBluetoothCombinedSpeakerReport != null &&
                    latestBluetoothCombinedSpeakerReport.Length == BluetoothCombinedOutputReportLength)
                {
            ...
            if (enabled)
            {
                report[BluetoothCombinedAudioControlFlagsOffset] =
                    (byte)((report[BluetoothCombinedAudioControlFlagsOffset] |
                        BluetoothCombinedAudioControlFlagsBase) |
                        BluetoothCombinedAudioControlMicrophoneEnable);
            }
            else
